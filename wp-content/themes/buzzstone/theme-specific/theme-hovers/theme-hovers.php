...
if ( ! function_exists( 'buzzstone_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'buzzstone_hovers_theme_setup3', 3 );
	function buzzstone_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		buzzstone_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'buzzstone' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'buzzstone' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'buzzstone' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'buzzstone' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'buzzstone' ) ),
					'std'      => 'simple',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'buzzstone' ),
					),
					'options'  => buzzstone_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'buzzstone_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'buzzstone_hovers_theme_setup9', 9 );
	function buzzstone_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'buzzstone_hovers_frontend_scripts', 1010 );
		add_filter( 'buzzstone_filter_localize_script', 'buzzstone_hovers_localize_script' );
		add_filter( 'buzzstone_filter_merge_scripts', 'buzzstone_hovers_merge_scripts' );
		add_filter( 'buzzstone_filter_merge_styles', 'buzzstone_hovers_merge_styles' );
		add_filter( 'buzzstone_filter_merge_styles_responsive', 'buzzstone_hovers_merge_styles_responsive' );
		add_filter( 'buzzstone_filter_get_css', 'buzzstone_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'buzzstone_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'buzzstone_hovers_frontend_scripts', 1010 );
	function buzzstone_hovers_frontend_scripts() {
		if ( buzzstone_is_on( buzzstone_get_theme_option( 'debug_mode' ) ) ) {
			$buzzstone_url = buzzstone_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $buzzstone_url ) {
				wp_enqueue_script( 'buzzstone-hovers', $buzzstone_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'buzzstone_hovers_merge_styles' ) ) {
	//Handler of the add_filter( 'buzzstone_filter_merge_styles', 'buzzstone_hovers_merge_styles' );
	function buzzstone_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'buzzstone_hovers_merge_styles_responsive' ) ) {
	//Handler of the add_filter( 'buzzstone_filter_merge_styles_responsive', 'buzzstone_hovers_merge_styles_responsive' );
	function buzzstone_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'buzzstone_hovers_localize_script' ) ) {
	//Handler of the add_filter( 'buzzstone_filter_localize_script','buzzstone_hovers_localize_script' );
	function buzzstone_hovers_localize_script( $arr ) {
		$arr['button_hover'] = buzzstone_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'buzzstone_hovers_merge_scripts' ) ) {
	//Handler of the add_filter( 'buzzstone_filter_merge_scripts', 'buzzstone_hovers_merge_scripts' );
	function buzzstone_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'buzzstone_hovers_add_icons' ) ) {
	function buzzstone_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php buzzstone_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'buzzstone' ) : esc_html__( 'Buy now', 'buzzstone' ) ) : '' )
										. '</a>',
							$product
						)
					);
                    ...
                    <a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>"
                       aria-hidden="true" class="shop_link button icon-link">
                        ...
                        if ('shop_buttons' == $hover) {
                            if (is_object($args['cat'])) {
                                esc_html_e('View products', 'buzzstone');
                            } else {
                                esc_html_e('Details', 'buzzstone');
                            }
                        }
                        ?>
                    </a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php buzzstone_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php buzzstone_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php

		} elseif ( 'simple' == $hover ) {
            // Hover style 'Dots'
            ?>
            <a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php buzzstone_show_layout($target); ?> aria-hidden="true" class="icons"></a>
            <?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php buzzstone_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php buzzstone_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'buzzstone_hovers_get_css' ) ) {
	//Handler of the add_filter( 'buzzstone_filter_get_css', 'buzzstone_hovers_get_css', 10, 2 );
	function buzzstone_hovers_get_css( $css, $args ) {
