...
if ( ! function_exists( 'buzzstone_need_page_title' ) ) {
	function buzzstone_need_page_title() {
		return ! is_front_page() && apply_filters( 'buzzstone_filter_need_page_title', true );
	}
}
...
if ( ! function_exists( 'buzzstone_show_layout' ) ) {
	function buzzstone_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'buzzstone_get_logo_image' ) ) {
	function buzzstone_get_logo_image( $type = '' ) {
		$logo_image  = '';
		if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
			$logo_image = buzzstone_get_theme_option( 'custom_logo' );
			if ( empty( $logo_image ) ) {
			...
			$logo_image = buzzstone_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
		}
		$logo_retina = buzzstone_is_on( buzzstone_get_theme_option( 'logo_retina_enabled' ) )
						? buzzstone_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' )
						: '';
		return array(
					'logo'        => ! empty( $logo_image ) ? buzzstone_remove_protocol_from_url( $logo_image, false ) : '',
					'logo_retina' => ! empty( $logo_retina ) ? buzzstone_remove_protocol_from_url( $logo_retina, false ) : ''
				);
	}
}

// Return header video (if set)
if ( ! function_exists( 'buzzstone_get_header_video' ) ) {
	function buzzstone_get_header_video() {
		$video = '';
		if ( apply_filters( 'buzzstone_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( buzzstone_check_theme_option( 'header_video' ) ) {
				$video = buzzstone_get_theme_option( 'header_video' );
				if ( (int) $video > 0 ) {
...
// Show post banner
if ( ! function_exists( 'buzzstone_show_post_banner' ) ) {
	function buzzstone_show_post_banner( $banner_pos = '') {
		if ( is_singular( 'post' ) && '' !== $banner_pos ){
			$banner_code = buzzstone_get_theme_option( $banner_pos . '_banner_code' );
			$banner_img = buzzstone_get_theme_option( $banner_pos . '_banner_img' );
			$banner_class = !empty( $banner_img )
								? ( 'background' == $banner_pos ? '' : 'banner_with_image ' ) . buzzstone_add_inline_css_class( 'background-image:url(' . esc_url( $banner_img ) . ')' )
								: '';
			$banner_link = buzzstone_get_theme_option( $banner_pos . '_banner_link' );
			if ( ! empty( $banner_code ) || ! empty( $banner_img ) ) {
				...
				buzzstone_show_layout( $banner_code );
...
if ( ! function_exists( 'buzzstone_show_post_featured_image' ) ) {
	function buzzstone_show_post_featured_image() {
		$seo = buzzstone_is_on( buzzstone_get_theme_option( 'seo_snippets' ) );
		// Featured image
		if ( buzzstone_is_off( buzzstone_get_theme_option( 'hide_featured_on_single' ) )
					&& ! buzzstone_sc_layouts_showed( 'featured' )
					&& strpos( get_the_content(), '[trx_widget_banner]' ) === false ) {
			do_action( 'buzzstone_action_before_post_featured' );
			buzzstone_show_post_featured( array( 'thumb_bg' => buzzstone_get_theme_option( 'post_thumbnail_type' ) == 'fullwidth' ) );
			do_action( 'buzzstone_action_after_post_featured' );
...
if ( ! function_exists( 'buzzstone_show_post_title_and_meta' ) ) {
	function buzzstone_show_post_title_and_meta() {

		$seo = buzzstone_is_on( buzzstone_get_theme_option( 'seo_snippets' ) );

		// Title and post meta
		if ( ( ! buzzstone_sc_layouts_showed( 'title' )
		|| ! buzzstone_sc_layouts_showed( 'postmeta' ) )
		) {
			do_action( 'buzzstone_action_before_post_title' );
			$need_content_wrap = buzzstone_get_theme_option( 'post_thumbnail_type' ) == 'fullwidth' && buzzstone_get_theme_option( 'post_header_position' ) !== 'on_thumb';
			?>
			<div class="post_header post_header_single entry-header">
				...
				// Post meta
				if ( ! buzzstone_sc_layouts_showed( 'postmeta' ) && buzzstone_is_on( buzzstone_get_theme_option( 'show_post_meta' ) ) ) {
					buzzstone_show_post_meta(
						apply_filters(
							'buzzstone_filter_post_meta_args',
							array(
				...
				// Post excerpt
				if ( buzzstone_is_on( buzzstone_get_theme_option( 'show_post_excerpt' ) ) && has_excerpt() ) {
					?>
			...
			<?php
			do_action( 'buzzstone_action_after_post_title' );
		}
	}
}
...
if ( ! function_exists( 'buzzstone_show_post_meta' ) ) {
	function buzzstone_show_post_meta( $args = array() ) {
		if ( is_single() && buzzstone_is_off( buzzstone_get_theme_option( 'show_post_meta' ) ) ) {
		...
		<div class="post_meta<?php echo ! empty( $args['class'] ) ? ' ' . esc_attr( $args['class'] ) : ''; ?>">
			...
			foreach ( $components as $comp ) {
				...
				if ( 'categories' == $comp ) {
					...
					if ( ! empty( $cats ) ) {
						?>
						<span class="post_meta_item post_categories"><?php buzzstone_show_layout( $cats ); ?></span>
						<?php
					}
				} elseif ( 'date' == $comp ) {
					// Post date
					$dt = apply_filters( 'buzzstone_filter_get_post_date', buzzstone_get_date() );
					if ( ! empty( $dt ) ) {
						?>
					...
					// Post counters
					$output = buzzstone_get_post_counters( $args['counters'] );
					if ( '' != $output ) {
						buzzstone_show_layout( $output );
					} elseif ( ! buzzstone_exists_trx_addons() ) {
						if ( ! is_singular() || have_comments() || comments_open() ) {
							...
							?>
								<a href="<?php echo esc_url( get_comments_link() ); ?>" class="post_meta_item post_counters_item post_counters_comments icon-comment-light"><span class="post_counters_number">
									...
									<?php
									echo wp_kses_data(($post_comments > 1) ? esc_html__('Comments', 'buzzstone') : esc_html__('Comment', 'buzzstone'));
									?>
									</span>
								</a>
							<?php
					...
					buzzstone_show_share_links(
						array(
							'type'    => 'drop',
							'caption' => esc_html__( 'Share', 'buzzstone' ),
					...
					// Edit page link
					edit_post_link( esc_html__( 'Edit', 'buzzstone' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );
				} else {
					// Custom counter
					do_action( 'buzzstone_action_show_post_counter', $comp, get_the_ID() );
				}
			}
			?>
		</div><!-- .post_meta -->
...
if ( ! function_exists( 'buzzstone_show_post_featured' ) ) {
	function buzzstone_show_post_featured( $args = array() ) {
		$args = array_merge(
			array(
				'hover'         => buzzstone_get_theme_option( 'image_hover' ), // Hover effect
		...
		$thumb_size  = ! empty( $args['thumb_size'] )
						? $args['thumb_size']
						: buzzstone_get_thumb_size( is_attachment() || is_single() ? 'full' : 'big' );
		$post_format = str_replace( 'post-format-', '', get_post_format() );
		$no_image    = ! empty( $args['show_no_image'] ) ? buzzstone_get_no_image() : '';
		if ( $args['thumb_bg'] ) {
			if ( has_post_thumbnail() ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), $thumb_size );
				$image = $image[0];
			} elseif ( 'image' == $post_format ) {
				$image = buzzstone_get_post_image();
				if ( ! empty( $image ) ) {
					$image = buzzstone_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				$args['class'] .= ( $args['class'] ? ' ' : '' ) . 'post_featured_bg' . ' ' . buzzstone_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			}
		}
		...
		if ( $args['singular'] ) {
			...
			if ( is_attachment() ) {
				...
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
						...
						buzzstone_is_on( buzzstone_get_theme_option( 'seo_snippets' ) )
													? array( 'itemprop' => 'image' )
						: ''
					);
				}
				if ( buzzstone_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				if ( has_post_thumbnail() && $args['seo'] ) {
					$buzzstone_attr = buzzstone_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
					?>
						<meta itemprop="width" content="<?php echo esc_attr( $buzzstone_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $buzzstone_attr[1] ); ?>">
						<?php
				}
				if ( ! $args['thumb_bg'] ) {
					if ( has_post_thumbnail() ) {
			...
			if ( $has_thumb	|| ! empty( $args['show_no_image'] )
				...
				if ( $args['thumb_bg'] ) {
					...
					$output = buzzstone_get_slider_layout( $slider_args );
					if ( '' != $output ) {
						buzzstone_show_layout( $output );
					}
					//------- End: Moved down --------
				...
				if ( ! $args['thumb_only'] && ( in_array( $post_format, array( 'video', 'audio', 'gallery' ) ) ) ) {
					$post_content = buzzstone_get_post_content();
					$post_content_parsed = $post_content;
					// Put video under the thumb
					if ( 'video' == $post_format ) {
						$video = buzzstone_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = buzzstone_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = buzzstone_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							if ( $has_thumb ) {
								$video = buzzstone_make_video_autoplay( $video );
								?>
						...
						$audio = buzzstone_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = buzzstone_get_post_iframe( $post_content, false );
						}
						...
						if ( empty( $audio ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = buzzstone_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								$media = urldecode( buzzstone_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
								$media_author = '';
								$media_title  = '';
								if ( ! empty( $media ) ) {
									...
									// Parse tag params
									$media_author = strip_tags( buzzstone_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
									$media_title  = strip_tags( buzzstone_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );

								}
								if ( ! empty( $media_author ) ) {
									?>
									<div class="post_audio_author"><?php buzzstone_show_layout( $media_author ); ?></div>
									<?php
								}
								if ( ! empty( $media_title ) ) {
									?>
									<h5 class="post_audio_title"><?php buzzstone_show_layout( $media_title ); ?></h5>
									<?php
								}
								// Display audio
								buzzstone_show_layout( $audio );
								?>
							</div>
							<?php
						}
				...
				buzzstone_show_layout( $post_info );
...
if ( ! function_exists( 'buzzstone_get_no_image' ) ) {
	function buzzstone_get_no_image( $no_image = '' ) {
		static $img = '';
		if ( empty( $img ) ) {
			$img = buzzstone_get_theme_option( 'no_image' );
			if ( empty( $img ) ) {
				$img = buzzstone_get_file_url( 'images/no-image.jpg' );
			}
		}
		if ( ! empty( $img ) ) {
			$no_image = $img;
...
if ( ! function_exists( 'buzzstone_add_bg_in_post_nav' ) ) {
	function buzzstone_add_bg_in_post_nav() {
		if ( ! is_single() ) {
			return;
		}
		...
		if ( $previous ) {
			...
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), buzzstone_get_thumb_size( 'med' ) );
				$img = $img[0];
			} elseif ( buzzstone_get_theme_setting( 'allow_no_image' ) ) {
				$img = $noimg;
			}
			if ( ! empty( $img ) ) {
		...
		if ( $next ) {
			...
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), buzzstone_get_thumb_size( 'med' ) );
				$img = $img[0];
			} elseif ( buzzstone_get_theme_setting( 'allow_no_image' ) ) {
				$img = $noimg;
			}
		...
		wp_add_inline_style( 'buzzstone-main', $css );
	}
}

// Show related posts
if ( ! function_exists( 'buzzstone_show_related_posts' ) ) {
	function buzzstone_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : buzzstone_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int) buzzstone_get_theme_option( 'related_slider' ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			?>
			<section class="related_wrap">
				...
				if ( ! empty( $title ) ) {
				    ...
				    echo '<span>';
					esc_html_e( 'You May Also Like', 'buzzstone' );
				    echo '</span>';
				}
				?></h3><?php
				if ( $args['slider'] ) {
					...
					$slider_args['slides_space']      = buzzstone_get_theme_option( 'related_slider_space' );
					$slider_args['slider_controls']   = buzzstone_get_theme_option( 'related_slider_controls' );
					$slider_args['slider_pagination'] = buzzstone_get_theme_option( 'related_slider_pagination' );
					$slider_args                      = apply_filters( 'buzzstone_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					buzzstone_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						get_template_part( apply_filters( 'buzzstone_filter_get_template_part', 'templates/related-posts', $style ), $style );
						?></div><?php
					}
				?></div><?php	
				if ( $args['slider'] ) {
					buzzstone_get_slider_wrap_end('related_posts_wrap', $slider_args);
...
if ( ! function_exists( 'buzzstone_show_related_posts_callback' ) ) {
	add_action( 'buzzstone_action_related_posts', 'buzzstone_show_related_posts_callback' );
	function buzzstone_show_related_posts_callback() {
		if ( is_single() && ! apply_filters( 'buzzstone_filter_show_related_posts', false ) ) {
			$buzzstone_related_posts    = (int) buzzstone_get_theme_option( 'related_posts' );
			if ( (int) buzzstone_get_theme_option( 'show_related_posts' ) && $buzzstone_related_posts > 0 ) {
				buzzstone_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $buzzstone_related_posts ) ),
						'columns'        => max( 1, min( 4, buzzstone_get_theme_option( 'related_columns' ) ) ),
					),
					buzzstone_get_theme_option( 'related_style' )
				);
			}
		}
	}
...
if ( ! function_exists( 'buzzstone_show_portfolio_posts' ) ) {
	function buzzstone_show_portfolio_posts( $args = array() ) {
		$args = array_merge(
			array(
				'cat'        => 0,
				'parent_cat' => 0,
		...
		$blog_style = explode( '_', empty( $args['blog_style'] ) ? buzzstone_get_theme_option( 'blog_style' ) : $args['blog_style'] );
		$style      = $blog_style[0];
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = buzzstone_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
				$q_args['paged']               = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = buzzstone_get_theme_option( 'posts_per_page' );
			if ( 0 != (int) $ppp ) {
		...
		while ( have_posts() ) {
			...
			$buzzstone_part = $args['sticky'] && is_sticky() ? 'sticky' : ( 'gallery' == $style ? 'portfolio-gallery' : $style );
...
// AJAX handler for the buzzstone_ajax_get_posts action
if ( ! function_exists( 'buzzstone_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_buzzstone_ajax_get_posts', 'buzzstone_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_buzzstone_ajax_get_posts', 'buzzstone_ajax_get_posts_callback' );
	function buzzstone_ajax_get_posts_callback() {
		...
		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'buzzstone' );
		}
...
// Show pagination
if ( ! function_exists( 'buzzstone_show_pagination' ) ) {
	function buzzstone_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = buzzstone_get_theme_option( 'blog_pagination' );
		if ( 'pages' == $pagination ) {
			the_posts_pagination(
				array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( 'Prev', 'buzzstone' ),
					'next_text'          => esc_html__( 'Next', 'buzzstone' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'buzzstone' ) . ' </span>',
				)
			);
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e( 'Load More', 'buzzstone' ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'buzzstone' ) : esc_html__( 'Newest posts', 'buzzstone' ) ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'buzzstone' ) : esc_html__( 'Older posts', 'buzzstone' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'buzzstone_single_comments_field' ) ) {
	function buzzstone_single_comments_field( $args ) {
