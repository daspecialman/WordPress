...
if ( ! function_exists( 'buzzstone_customizer_get_css' ) ) {

	function buzzstone_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts = buzzstone_customizer_add_theme_fonts( $fonts );
...
body .minimal-light .esg-filterbutton, body .minimal-light .esg-navigationbutton, body .minimal-light .esg-sortbutton, body .minimal-light .esg-cartbutton a,
body .mejs-container *,
blockquote,
mark, ins,
.logo_text,
...
CSS;
			$rez          = apply_filters( 'buzzstone_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			// Make theme-specific fonts rules
			$vars = buzzstone_customizer_add_theme_vars( $vars );
			extract( $vars );
...
CSS;
			$rez          = apply_filters( 'buzzstone_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars']  = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( buzzstone_get_sorted_schemes() ) : array( $scheme );
			...
			if ( count( $schemes ) > 0 ) {
				...
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = buzzstone_get_scheme_colors( $s );
					}
...
table th, table th + th, table td + th  {
	border-color: {$colors['alter_bd_color_02']};
}
...
.wp-caption-overlay .wp-caption .wp-caption-dd {
	color: {$colors['inverse_link']};
	background: {$colors['text_link_08']};
	background: -moz-linear-gradient(left, {$colors['text_link_08']} 0%, {$colors['text_link2_08']} 100%);
	background: -webkit-linear-gradient(left, {$colors['text_link_08']} 0%,{$colors['text_link2_08']} 100%);
	background: linear-gradient(to right, {$colors['text_link_08']} 0%,{$colors['text_link2_08']} 100%);
}
figure figcaption a {
	color: {$colors['inverse_dark']}
}
...
.wrap-nav-info .select_container:before {
	color: {$colors['input_text']};
	background-color: {$colors['input_bg_color']};
}
...
.wrap-nav-info .select_container select {
	color: {$colors['input_text']};
...
.sidebar_inner .select_container:focus:before,
.sidebar_inner .select_container:hover:before {
	background-color: {$colors['alter_bg_hover']};
}


.scheme_self.sidebar .select_container:before,
.scheme_self.sidebar .select_container:after {
	background: {$colors['input_bg_color']} !important;
}
...
.footer_widgets_wrap .select_container:after {
	background: {$colors['input_bg_color']} !important;
}
.footer_widgets_wrap .select_container select {
	background: {$colors['input_bg_color']} !important;
}
.footer_widgets_wrap .select_container select:focus {
	color: {$colors['input_dark']};
	background-color: {$colors['input_bg_hover']} !important;
}
...
.buzzstone_tabs .buzzstone_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_dark']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.buzzstone_tabs .buzzstone_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.sheme_self.sidebar .buzzstone_tabs .buzzstone_tabs_titles li a {
	color: {$colors['alter_dark']};
	background-color: {$colors['alter_bg_hover']};
}
.sheme_self.sidebar div.esg-filter-wrapper .esg-filterbutton > span:hover,
.sheme_self.sidebar .mptt-navigation-tabs li a:hover,
.sheme_self.sidebar .buzzstone_tabs .buzzstone_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['alter_link']};
}
...
.sheme_self.sidebar .buzzstone_tabs .buzzstone_tabs_titles li.ui-state-active a {
	color: {$colors['alter_bg_color']};
	background-color: {$colors['alter_dark']};
}
...
.widget_calendar table {
	...
	outline-color: {$colors['alter_bd_color']};
	background-color: {$colors['alter_bg_color']};
}
...
.content .top_panel_title .sc_layouts_title_caption .title_top_icon {
	color: {$colors['text_link']};
}
...
.sidebar_inner .widget_title .widget-title-wrap {
	border-color: {$colors['text_link']};
}
...
.info_post_tags > a:hover {
	background-color: {$colors['text_dark']};
	color: {$colors['alter_bg_color']};
}
.post_item.post_layout_extra,
.wrap_post_content {
	background-color: {$colors['alter_bg_color']};
}
...
.scheme_self.thumbnail_type_fullwidth.post_header_position_under .header_content_wrap.header_align_mc,
.thumbnail_type_fullwidth.post_header_position_under .header_content_wrap.header_align_mc,

.scheme_self.thumbnail_type_fullwidth.post_header_position_above .header_content_wrap.header_align_mc,
.thumbnail_type_fullwidth.post_header_position_above .header_content_wrap.header_align_mc {
	background-color: {$colors['bg_color']};
}

.scheme_self.post_header_position_on_thumb .header_align_bs .post_featured + .post_header,
...
.menu_mobile_inner .menu_mobile_nav_area > ul > li + li {
	border-color: {$colors['bd_color']};
}
...
CSS;
					...
					$rez            = apply_filters(
						'buzzstone_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'buzzstone_filter_prepare_css', $css_str, $remove_spaces );
	}
}
