...
$number = min(6, $widget_args['number']);
$count = min(6, $widget_args['count']);
...
$grid = array(
    ...
    array('small', 'small', 'small', 'small'),
    array('small', 'small', 'small', 'small', 'small'),
    array('small', 'small', 'small', 'small', 'small', 'small'),
);
$thumb_size = $grid[$count-$number >= 6 ? 6 : ($count-1)%6][($number-1)%6];
    ...
    trx_addons_get_template_part('templates/tpl.featured.php',
        ...
        apply_filters('trx_addons_filter_args_featured', array(
            'post_info' =>  !in_array($post_format, array('link', 'aside', 'status', 'quote', 'audio')) ? '<div class="post_info">'
                ...
                . '</div>' : '',
            'thumb_bg' => true,
            'thumb_size' => buzzstone_get_thumb_size($number > 2 ? 'extra' : 'huge')
        ), 'recent_news-simple')
    );
    ?>
</article>