<div class="front_page_section front_page_section_team<?php
	$buzzstone_scheme = buzzstone_get_theme_option( 'front_page_team_scheme' );
	if ( ! buzzstone_is_inherit( $buzzstone_scheme ) ) {
		echo ' scheme_' . esc_attr( $buzzstone_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( buzzstone_get_theme_option( 'front_page_team_paddings' ) );
?>"
		<?php
		$buzzstone_css      = '';
		$buzzstone_bg_image = buzzstone_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$buzzstone_anchor_icon = buzzstone_get_theme_option( 'front_page_team_anchor_icon' );
	$buzzstone_anchor_text = buzzstone_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( buzzstone_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$buzzstone_css      = '';
			$buzzstone_bg_mask  = buzzstone_get_theme_option( 'front_page_team_bg_mask' );
			$buzzstone_bg_color_type = buzzstone_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $buzzstone_bg_color_type ) {
				$buzzstone_bg_color = buzzstone_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$buzzstone_caption = buzzstone_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $buzzstone_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $buzzstone_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( $buzzstone_caption ); ?></h2>
				<?php
			}

			// Description (text)
			$buzzstone_description = buzzstone_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $buzzstone_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $buzzstone_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( wpautop( $buzzstone_description ) ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
				...
				if ( ! buzzstone_exists_trx_addons() ) {
					...
					buzzstone_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
				}
			}
			?>
			</div>
